...
function Invoke-LaunchScreen {
    ...
    param(
        ...
        # from. Only .AvailableModels is read here, to skip a hidden Model/Advisor option when
        # stepping or clicking; the empty default keeps every existing caller and test unfiltered.
        [hashtable]$Limits = @{}
    )
    ...
    $walkRow = {
        ...
        # This account's model-bucket families, so a Left/Right over the Model/Advisor row skips a
        # hidden option instead of landing on it (spec 2026-09-17). The Account row itself is never
        # filtered, so its own step is unaffected; @() when there is no record (fail safe).
    ...
    return (Invoke-ScreenLoop -Screen 'launch' -State $st -Wait $Wait -GetWindowTop $GetWindowTop `
        ...
        Click = {
            ...
            # (spec 2026-09-17). The step count must match the visible-stepping walkRow, or a click
            # would over- or under-shoot the option it landed on.
            $available = @($launchLimits[$s.State.Account].AvailableModels)
