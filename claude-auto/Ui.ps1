...
function Invoke-LaunchScreen {
    ...
    param(
        ...
        # from. Only .Plan is read here, to skip a Model/Advisor option the plan does not include
        # when stepping or clicking; the empty default keeps every existing caller and test unfiltered.
        [hashtable]$Limits = @{}
    )
    ...
    $walkRow = {
        ...
        # This account's plan, so a Left/Right over the Model/Advisor row skips an option the plan
        # does not include instead of landing on it (spec 2026-09-17 v2). The Account row itself is
        # never filtered, so its own step is unaffected; '' when there is no record (fail safe).
        $plan = "$($launchLimits[$s.State.Account].Plan)"
        for ($n = 0; $n -lt $Count; $n++) { $s.State = Step-LaunchValue -State $s.State -Delta $Delta -Plan $plan }
    ...
    return (Invoke-ScreenLoop -Screen 'launch' -State $st -Wait $Wait -GetWindowTop $GetWindowTop `
        ...
        Click = {
            ...
            # Over the VISIBLE values, the same list that built the row's cells: a hidden option has
            # no cell, so a click at its former column finds no value here and the walk is a no-op
            # (spec 2026-09-17 v2). The step count must match the visible-stepping walkRow, or a
            # click would over- or under-shoot the option it landed on.
            $plan = "$($launchLimits[$s.State.Account].Plan)"
            $values = @(Get-VisibleRowValues -Row (Get-LaunchRows)[$hit.Row.Index] -Plan $plan)
